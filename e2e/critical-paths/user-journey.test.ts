...
test.describe('Critical User Journey', () => {
	test.fixme('Core user workflow: Register → Create Org → Create Item → Logout → Login', async ({
		page,
		registerPage,
		loginPage,
		homePage,
	...
	test.fixme('Basic owner workflow validation', async ({
		page,
		registerPage,
		homePage,
		organizationPage,
		...
		await registerPage.goto();
		await registerPage.registerAndWaitForRedirect(ownerEmail, password, password, organizationName);
