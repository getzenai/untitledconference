...
export function describeBulkAssign(result: {
	...
	already: number;
	skipped: number;
	/** Seats left recused — not restored by bulk (organizer must flip them one-by-one). */
	recused?: number;
}): string {
	const parts: string[] = [];
	if (result.created > 0) {
	...
	if ((result.recused ?? 0) > 0) {
		parts.push(
			`${result.recused} recused ${result.recused === 1 ? 'seat' : 'seats'} left alone — flip each on the submission if you mean to override.`
		);
	}
	if (result.skipped > 0) {
