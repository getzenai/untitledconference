...
describe('listPublishedConferences', () => {
	it('omits a conference the organizer has not published', async () => {
		const slugs = (await listPublishedConferences()).map((c) => c.slug);
	...
	it('carries what a card needs to be recognised and nothing else', async () => {
		const row = (await listPublishedConferences()).find((c) => c.slug === soonerSlug)!;
