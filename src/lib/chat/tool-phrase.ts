 ...
 * list/get) plus the rest of the name. Context is a name, title or id from
 * the arguments when one is present.
 */
...
export function toolObject(name: string): string {
	...
	return words.slice(1).join(' ') || name.replace(/[_-]+/g, ' ').trim() || name;
}
...
export function toolPhrase(name: string, state: ToolUIPartState): string {
	...
	if (state === 'output-denied') return `${capitalize(object)} — not done`;
	if (state === 'output-error') {
		return LOOKUP.has(verb) ? `Couldn't look up ${object}` : `Couldn't ${verb} ${object}`;
	}

	if (LOOKUP.has(verb)) return live(state) ? `Looking up ${object}` : capitalize(object);
	if (live(state)) return `${capitalize(gerund(verb))} ${object}`;
	return `${capitalize(past(verb))} ${object}`;
}
