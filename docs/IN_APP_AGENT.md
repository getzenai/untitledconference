...
API key of *their* active organization, not the organizer's. That is the
...
the rule. Rows marked **form only** are the debt. Stufe 2 pays two
clusters of that debt (form builder, schedule fill) and leaves the rest
visible.
...
| Erase (archived, never published) | (settings, after archive) | `deleteConference` | `delete_conference` |
...
`open_cfp` / `close_cfp` change the call's *status*. They do not edit
...
`swap_talks` already exists. Fill does not. `create_break` is older than
the screen: `docs/MCP.md` still says "there is no screen for this", but
`hold` has been the same `createBlock` since #450. The tool is not the
gap.
