...
| `fill_schedule`                                  | Pack the tray into free slots — same filler as the Fill button. Leftovers stay in the tray             |
...
there is no grid to look at, so the refusal is the flag. `fill_schedule` is the
other shape: talks the packer cannot fit stay in the tray and stay readable with
`get_agenda_tray` — that leftover is not an error.

## Speaker

| Tool                        | What it does                                                               |
| --------------------------- | -------------------------------------------------------------------------- |
| `list_open_cfps`            | Calls currently accepting proposals, with each format's length             |
| `submit_proposal`           | Create a draft. It is not handed in — the return names `finalize_proposal` |
...
A speaker profile belongs to one organizer, not to the account globally — you
have one per organization you have spoken for. Pass `conferenceSlug` to
`update_my_speaker_profile` to reach a particular one, or to create your first
before you have submitted anything.
