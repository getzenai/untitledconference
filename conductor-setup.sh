...
if command -v nvm &> /dev/null; then
    ...
    nvm use 22 || {
        echo "⚠️  Warning: Could not switch to Node 22, using current Node version: $(node -v)"
    }
else
...
if command -v docker &> /dev/null; then
    if ! docker ps --format '{{.Names}}' | grep -q 'postgres-vibe-starter$'; then
        echo "⚠️  Warning: Database container 'postgres-vibe-starter' not detected"
        echo "Make sure Docker containers are running: npm run db:start"
    fi
else
    echo "⚠️  Warning: Docker not found. Ensure database containers are running."
fi
